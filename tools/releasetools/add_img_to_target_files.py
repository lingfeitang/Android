...
def AddProduct(output_zip,prefix="IMAGES/"):
  """Create an empty product image and store it in output_zip."""

  #prebuilt_path = os.path.join(OPTIONS.input_tmp, prefix, "prodnv.img")

  image_props = build_image.ImagePropFromGlobalDict(OPTIONS.info_dict,
                                                    "productinfo")
  # The build system has to explicitly request for product.img.
  if "fs_type" not in image_props:
    return

  print "creating prodnv.img..."

  # The name of the directory it is making an image out of matters to
  # mkyaffs2image.  So we create a temp dir, and within it we create an
  # empty dir named "product", and build the image from that.
  temp_dir = tempfile.mkdtemp()
  user_dir = os.path.join(temp_dir, "productinfo")
  os.mkdir(user_dir)
  img = tempfile.NamedTemporaryFile()

  fstab = OPTIONS.info_dict["fstab"]
  if fstab:
    image_props["fs_type" ] = fstab["/productinfo"].fs_type
  succ = build_image.BuildImage(user_dir, image_props, img.name)
  assert succ, "build prodnv.img image failed"

  common.CheckSize(img.name, "prodnv.img", OPTIONS.info_dict)
  common.ZipWrite(output_zip, img.name, prefix + "prodnv.img")
  img.close()
  os.rmdir(user_dir)
  os.rmdir(temp_dir)
