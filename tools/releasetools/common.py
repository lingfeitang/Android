...
def LoadInfoDict(input_file, input_dir=None):
  ...
  # SPRD: convert persist_size from string to int type @ {
  makeint("persist_size")
  # @ }
...
def CheckSize(data, target, info_dict):
  ...
  # SPRD: modify percent of make image parameter and image size creatd to be uniform @ {
  if pct > 100.0:
  # @ }
...
""" % {'type': recovery_type,
    ...
    if recovery_type == "EMMC":
      recovery_device = "/dev/block/platform/sdio_emmc/by-name/recovery"
      boot_device = "/dev/block/platform/sdio_emmc/by-name/boot"
    sh = """#!/system/bin/sh
...
""" % {'boot_size': boot_img.size,
  ...
  # SPRD: no more need to make sure the path of install-recovery.sh @ {
  try:
    init_rc_files = os.listdir(init_rc_dir)
  except (OSError, IOError) as e:
    print "failed to read init.rc"
