...
def AdjustPartitionSizeForVerity(partition_size, fec_supported, origin_in, prop_dict):
  ...
  veritysize = GetVeritySize(result, fec_supported)
  if veritysize + result != partition_size:
    result = ResizeImage(partition_size, fec_supported, origin_in, prop_dict)
    if not result:
      return False
...
def ResizeImage(partition_size, fec_supported, origin_in, prop_dict):
  print("sprd add ResizeImage")
  total_size = partition_size
  while True:
    total_size = total_size + 50 * 1024 * 1024
    hi = total_size
    if hi % BLOCK_SIZE != 0:
      hi = (hi // BLOCK_SIZE) * BLOCK_SIZE
    lo = total_size - GetVeritySize(hi, fec_supported)
    result = lo
    while lo < hi:
      i = ((lo + hi) // (2 * BLOCK_SIZE)) * BLOCK_SIZE
      size = i + GetVeritySize(i, fec_supported)
      if size <= total_size:
        ...
        hi = i
    verify_size = GetVeritySize(result, fec_supported)
    if verify_size + result == total_size:
      print("sprd add total_size:", total_size, "result:", result, "verify_size:", verify_size)
      break

  system_image_size_adapt_support = prop_dict.get("system_image_size_adapt") != "false"
  print('system_image_size_adapt:',system_image_size_adapt_support)
  if system_image_size_adapt_support:
    path = os.path.split(origin_in)[0]
    mount_point = prop_dict["mount_point"]
    os.system('build/core/partition_size_self_readaption.sh %s %d' % (path, total_size))
  else:
    print('Error: BOARD_SYSTEMIMAGE_PARTITION_SIZE is not fit, Suggest BOARD_SYSTEMIMAGE_PARTITION_SIZE=%d', total_size)
    return False
...
def BuildImage(in_dir, prop_dict, out_file, target_out=None):
  ...
  if verity_supported and is_verity_partition:
    ...
    adjusted_size = AdjustPartitionSizeForVerity(partition_size, verity_fec_supported, origin_in, prop_dict)
...
def ImagePropFromGlobalDict(glob_dict, mount_point):
  ...
  if mount_point == "system":
    ...
    copy_prop("system_image_size_adapt", "system_image_size_adapt")
    #SPRD: add for make systemimage performance @ {
